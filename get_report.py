...
def open_chat(driver, chat_name):
    try:
        ...
        chat_element = WebDriverWait(driver, 15).until(
            EC.element_to_be_clickable((By.XPATH, f"//span[normalize-space(text())='{chat_name}']"))
        )
        chat_element.click()
            ...
            for row in data:
                try:
                    ...
                    if current_hour == 8:
                        ...
                        end = datetime.datetime.combine(now.date(), datetime.time(1, 30))
                        is_in_time = start <= full_datetime < end
                    elif current_hour == 14:
                        start = datetime.datetime.combine(now.date(), datetime.time(1, 0))
                        end = datetime.datetime.combine(now.date(), datetime.time(14, 0))
                        is_in_time = start <= full_datetime < end
                    else: 
