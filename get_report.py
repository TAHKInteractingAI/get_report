    ...
    try:
        chat_element = WebDriverWait(driver, 15).until(
            EC.element_to_be_clickable(
                (By.XPATH, f"//span[contains(text(), '{chat_name}')]")
            )
        )
        chat_element.click()
...
def preprocess_message(content):
    ...
    # Bước 2: Loại bỏ các mục 6, 7, 8, 9, 10 (theo yêu cầu)
    # Regex này tìm các dòng bắt đầu bằng + (6|7|8|9|10)/ và xóa đến hết dòng đó
    content = re.sub(r"^\+\s*(6|7|8|9|10)/.*$", "", content, flags=re.MULTILINE)

    # Bước 3: Xử lý xuống dòng và thêm ZWSP
...
def get_filtered_messages(current_hour):
    ...
    }  # Khởi tạo dictionary để lưu message theo sheet name
    EXCLUDED_SHEETS = [
        ...
        try:
            ...
            for row in data:
                try:
                    ...
                    time_str = str(
                        row["TIME"]
                    )  # Giả định định dạng HH:MM hoặc HH:MM:SS
                    ...
                    if current_hour == 8:
                        ...
                        end_time = datetime.datetime.combine(
                            now.date(), datetime.time(13, 0)
                        )  # 13h hôm nay
                        if start_time <= full_datetime < end_time:
                            messages[sheet_name].append(content)
                    else:
                        print(
                            f"Current time là {current_hour}h, không khớp với điều kiện lọc. (8h hoặc 14h)"
                        )
                    # else:
...
def create_or_update_report_sheet(messages):
    try:
        ...
        sheet_names_with_data = [
            sheet_name for sheet_name in sheet_names if messages[sheet_name]
        ]
        ...
        try:
            ...
            report_sheet = spreadsheet.add_worksheet(
                title="Report", rows="100", cols=len(sheet_names_with_data)
            )
        ...
        max_length = max(
            len(messages[sheet]) for sheet in sheet_names_with_data
        )  # Tìm chiều dài dài nhất của các message list
    ...
    try:
        ...
        try:
            ...
            actions.move_by_offset(
                500, 500
            ).click().perform()  # Click vào vị trí gần giữa
...
if __name__ == "__main__":
    ...
    print(f"\n✅ Báo cáo đã lọc (đã bỏ mục 6-10) lúc {current_hour}h:")
